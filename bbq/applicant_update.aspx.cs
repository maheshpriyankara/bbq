...
namespace bbq
{
    public partial class applicant_update : System.Web.UI.Page
    {
   ...
   WebConfigurationManager.ConnectionStrings["conn_job"].ConnectionString);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                db = new db();
                list_category.Items.Add("Applicant Dashboard");
                list_category.Items.Add("Interview Dashboard");
                ...
                {
                    conn_job.Open();
                    reader = new SqlCommand("select applicant_name from job_applicant where id='" + Session["applicantID"] + "'", conn_job).ExecuteReader();
                    if (reader.Read())
                    {
                        text_applicantName.Text = reader[0] + "";
                    }
                    conn_job.Close();
                }
                else
                {
                    Response.Redirect("applicant_dashboard.aspx");
                }
            }
        ...
        protected void btn_save_Click1(object sender, EventArgs e)
        {
            try
            {
                if (text_applicantName.Text.Equals(""))
                {
                    string script = "alert('Sory Invalied Applicant');";
                    ClientScript.RegisterClientScriptBlock(this.GetType(), "alert", script, true);
                }
                else if (text_description.Value.Equals(""))
                {
                    string script = "alert('Please Input Remark to Update');";
                    ClientScript.RegisterClientScriptBlock(this.GetType(), "alert", script, true);
                }
                else
                {
                    ...
                    conn_job.Open();
                    new SqlCommand("insert into applicant_remark values ('" + Session["applicantID"] + "','" + list_category.Text + "','" + text_description.InnerText + "','" + db.getSysDateTime() + "')", conn_job).ExecuteNonQuery();
                    conn_job.Close();
                    ...
                    try
                    {
                        ...
                        {
                            FileUpload.SaveAs(HttpContext.Current.Server.MapPath("~/upload/upload_") + Session["applicantID"] + ".jpeg");
                    ...
                    string script = "alert('Applicant Profile Updated Successfully'); window.close();";
                    ClientScript.RegisterClientScriptBlock(this.GetType(), "alert", script, true);
